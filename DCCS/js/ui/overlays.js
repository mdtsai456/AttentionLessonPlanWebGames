...
.dccs-overlay {
  ...
  font-family: "Nunito", "MaokenAssortedSans", "Microsoft JhengHei", sans-serif;
  color: #eee;
}
.dccs-overlay[hidden] {
...
.dccs-overlay-loading {
  ...
  background:
    ...
    url("../Home/img/HomeBack.png") center / cover no-repeat;
}
.dccs-overlay-loading .dccs-panel {
  min-width: min(78vw, 360px);
  padding: clamp(2rem, 5vh, 2.6rem) clamp(1.6rem, 4vw, 2.4rem);
  text-align: center;
  color: #2f5d3a;
  ...
  border-radius: 32px;
  box-shadow:
    0 10px 0 rgba(90, 170, 210, 0.22),
    0 22px 36px rgba(80, 140, 90, 0.18);
  backdrop-filter: blur(6px);
}
.dccs-overlay-loading .dccs-panel h1 {
  margin: 0 0 0.45rem;
  color: #3f8f4e;
...
.dccs-overlay-loading .dccs-hint {
  margin: 0;
  color: #5b6980;
...
.dccs-overlay-level .dccs-panel {
  ...
  justify-content: center;
  width: min(86%, 640px);
  min-height: min(52%, 360px);
  min-width: 0;
  max-width: none;
  padding: clamp(56px, 9vh, 88px) clamp(24px, 5vw, 56px) clamp(24px, 4vh, 40px);
  border: 3px solid rgba(255, 255, 255, 0.95);
  border-radius: 32px;
  background: rgba(255, 255, 255, 0.78);
  box-shadow:
...
.dccs-level-badge {
  ...
  min-width: clamp(160px, 42%, 240px);
  padding: clamp(10px, 1.8vh, 16px) clamp(22px, 4vw, 36px);
  border-radius: 999px;
  background: linear-gradient(#8ed8f6, #57b8e8);
  box-shadow: 0 6px 0 #3494c4, 0 10px 14px rgba(60, 140, 180, 0.28);
  color: #fff;
...
.dccs-level-continue {
  ...
  margin: 0;
  padding: clamp(10px, 1.8vh, 14px) 28px;
  border: 0;
  border-radius: 999px;
  background: linear-gradient(#9ee36a, #6ec94a);
  color: #fff;
  font: inherit;
  ...
  letter-spacing: 0.04em;
  cursor: pointer;
  box-shadow: 0 6px 0 #4aa43a, 0 10px 14px rgba(70, 140, 60, 0.28);
}
...
.dccs-level-continue:active {
  ...
  background: linear-gradient(#4fbe3c, #3a9a2c);
  box-shadow: none;
}
.dccs-break-actions {
  display: flex;
...
.dccs-break-actions .dccs-level-continue,
.dccs-break-lobby {
  width: 100%;
}
.dccs-break-lobby {
...
.dccs-break-lobby:hover,
.dccs-break-lobby:focus-visible {
  filter: brightness(1.04);
  outline: none;
}
.dccs-break-lobby:active {
  transform: translateY(4px);
  background: linear-gradient(#3aa3d0, #2b86b4);
  box-shadow: none;
}
.dccs-overlay-result .dccs-result-score {
...
.dccs-overlay-result .dccs-result-status {
  margin: 0 0 1rem;
  color: #5b6980;
...
.dccs-overlay-result .dccs-break-lobby {
  width: clamp(160px, 42%, 240px);
}
`;
...
export function createOverlays(container) {
  ...
  const breakEl = buildOverlay(
    'dccs-overlay-level dccs-overlay-break',
    '<div class="dccs-level-badge">挑戰完成！</div>' +
      '<p class="dccs-level-message">第 3 關已結束<br>要繼續遊玩嗎？</p>' +
      '<div class="dccs-break-actions">' +
        '<button class="dccs-level-continue" type="button" data-break="continue">繼續遊玩</button>' +
        '<button class="dccs-break-lobby" type="button" data-break="lobby">返回遊戲大廳</button>' +
      '</div>'
  );
  const resultEl = buildOverlay(
    'dccs-overlay-level dccs-overlay-result',
    '<div class="dccs-level-badge">挑戰完成！</div>' +
      '<p class="dccs-level-message dccs-result-score"></p>' +
      '<p class="dccs-result-detail"></p>' +
      '<p class="dccs-hint dccs-result-status"></p>' +
      '<button class="dccs-break-lobby" type="button" data-result="lobby">返回遊戲大廳</button>'
  );
  ...
  const resultLobbyButton = resultEl.querySelector('[data-result="lobby"]');
  let resolveLevelPrompt = null;
  let resolveBreakPrompt = null;
  let resolveResultLobby = null;
  ...
  function settleBreakPrompt(choice) {
    if (!resolveBreakPrompt) return;
    const resolve = resolveBreakPrompt;
    resolveBreakPrompt = null;
    resolve(choice);
  }

  breakContinueButton.addEventListener('click', () => settleBreakPrompt('continue'));
  breakLobbyButton.addEventListener('click', () => settleBreakPrompt('lobby'));

  function settleResultLobby() {
    if (!resolveResultLobby) return;
    const resolve = resolveResultLobby;
    resolveResultLobby = null;
    resolve();
  }

  resultLobbyButton.addEventListener('click', settleResultLobby);
  ...
  function showContinueOrLobby() {
    showOnly(breakEl);
    breakContinueButton.focus();
    return new Promise((resolve) => {
      resolveBreakPrompt = resolve;
    });
  }
  ...
  function showResult(summary, statusText) {
    const correct = summary && summary.correct_count != null ? summary.correct_count : 0;
    const accuracy = Math.round(((summary && summary.accuracy) || 0) * 100);
    const stage = summary && summary.stage != null ? summary.stage : 0;
    resultEl.querySelector('.dccs-result-score').textContent = `答對 ${correct} 題`;
    resultEl.querySelector('.dccs-result-detail').textContent =
      `正確率 ${accuracy}% · 到達第 ${stage} 關`;
    resultEl.querySelector('.dccs-result-status').textContent = statusText || '';
    showOnly(resultEl);
  }

  function waitForResultLobby() {
    resultLobbyButton.focus();
    return new Promise((resolve) => {
      resolveResultLobby = resolve;
    });
  }
