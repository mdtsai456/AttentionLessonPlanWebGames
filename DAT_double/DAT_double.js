...
function createPlayer(element, bindings, answerCodes, answerLabel) {
  ...
  function showQuestion() {
    ...
    $('animal').dataset.result = '';
    $('feedback').textContent = `題目正確就瞄準按 ${answerLabel}；不正確則不按`;
  ...
  return { startGame, tick };
}

// 玩家各自瞄準後開始，計時、準心、答案與分數互不影響。
const players = [
  createPlayer(document.querySelector('.player-1'),
    { KeyW: 'up', KeyS: 'down', KeyA: 'left', KeyD: 'right' }, ['Space'], '空白鍵'),
  createPlayer(document.querySelector('.player-2'),
    { ArrowUp: 'up', ArrowDown: 'down', ArrowLeft: 'left', ArrowRight: 'right' }, ['Enter', 'NumpadEnter'], 'Enter')
];
