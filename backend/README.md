...
## 測試庫登入帳號（`AttentionLessonPlan_test`）

本機前端接後端時，`.env` 的 `DB_NAME` 應指向 `AttentionLessonPlan_test`。
登入帳號是後端指派的 `S0001`／`T0001`，**不是**後台看到的 `G1_S01`。
`studentKey` = `年級_個案編號`（例如 `G1_S01`）；同一組個案編號在不同場域是不同學生。

學生由 `seed.py` 灌入，**18 位密碼都是 `test1234`**：

| 登入帳號 | 密碼 | 場域 | 年級 | 個案 | 後台 studentKey |
|---|---|---|---|---|---|
| S0001 | test1234 | KMU | G1 | S01 | G1_S01 |
| S0002 | test1234 | KMU | G1 | S02 | G1_S02 |
| S0003 | test1234 | KMU | G1 | S03 | G1_S03 |
| S0004 | test1234 | KMU | G2 | S04 | G2_S04 |
| S0005 | test1234 | KMU | G2 | S05 | G2_S05 |
| S0006 | test1234 | KMU | G2 | S06 | G2_S06 |
| S0007 | test1234 | NTHU-01 | G1 | S01 | G1_S01 |
| S0008 | test1234 | NTHU-01 | G1 | S02 | G1_S02 |
| S0009 | test1234 | NTHU-01 | G1 | S03 | G1_S03 |
| S0010 | test1234 | NTHU-01 | G2 | S04 | G2_S04 |
| S0011 | test1234 | NTHU-01 | G2 | S05 | G2_S05 |
| S0012 | test1234 | NTHU-01 | G2 | S06 | G2_S06 |
| S0013 | test1234 | NTHU-02 | G1 | S01 | G1_S01 |
| S0014 | test1234 | NTHU-02 | G1 | S02 | G1_S02 |
| S0015 | test1234 | NTHU-02 | G1 | S03 | G1_S03 |
| S0016 | test1234 | NTHU-02 | G2 | S04 | G2_S04 |
| S0017 | test1234 | NTHU-02 | G2 | S05 | G2_S05 |
| S0018 | test1234 | NTHU-02 | G2 | S06 | G2_S06 |

老師帳號由 `seed_directory.py` 依 `teacher_id` 編成 `T0001`…`T0016`。
本機已把 **KMU 吳老師** 重設為 `test1234`；其他老師若沒重設，密碼是灌資料時印出的隨機字串。

| 登入帳號 | 密碼 | 場域 | 老師 |
|---|---|---|---|
| T0001 | test1234 | KMU | 吳老師 |
| T0002 | （未重設則為隨機密碼） | KMU | 林老師 |
| T0003 | （未重設則為隨機密碼） | NTHU-01 | 王老師 |
| T0004 | （未重設則為隨機密碼） | NTHU-01 | 陳老師 |
| T0005 | （未重設則為隨機密碼） | NTHU-02 | 張老師 |
| T0006 | （未重設則為隨機密碼） | NTHU-02 | 李老師 |
| T0007 | （未重設則為隨機密碼） | NTHU-03 | 黃老師 |
| T0008 | （未重設則為隨機密碼） | NTHU-03 | 劉老師 |
| T0009 | （未重設則為隨機密碼） | NTHU-04 | 蔡老師 |
| T0010 | （未重設則為隨機密碼） | NTHU-04 | 楊老師 |
| T0011 | （未重設則為隨機密碼） | NTHU-05 | 許老師 |
| T0012 | （未重設則為隨機密碼） | NTHU-05 | 鄭老師 |
| T0013 | （未重設則為隨機密碼） | NTHU-06 | 謝老師 |
| T0014 | （未重設則為隨機密碼） | NTHU-06 | 郭老師 |
| T0015 | （未重設則為隨機密碼） | NTHU-07 | 洪老師 |
| T0016 | （未重設則為隨機密碼） | NTHU-07 | 曾老師 |

KMU 老師登入後，`/api/me/students` 只會看到該場域學生（S0001～S0006）。
重設其他老師密碼：

```bash
.\.venv\Scripts\python.exe manage_passwords.py teacher KMU 林老師 test1234
```
