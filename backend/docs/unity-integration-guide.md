...
**所有六個人都要確認**（見 §6）：你的 build 送的 `data.school` 字串，跟中介平台 /
前端用的**完全一致**。
...
中介平台登記的差一個字（全形半形、空白、破折號），**兩邊就永遠對不起來，而且不會
報錯**，只會查出空資料。

- 目前中介平台用**佔位代碼**：`KMU`、`NTHU-01` … `NTHU-07`（8 個場域）。
- 廠商的 **8 個正式場域字串還沒定案**。定案後每個場域的 Unity build 要送對應的那個字串。
- **請等一份「場域字串正式清單」出來後再把字串寫死進 build。** 在那之前先用佔位代碼開發測試。
...
- [ ] 用測試環境的佔位 `school` 代碼（如 `KMU`）先測通，收到 `201`
- [ ] 等正式場域字串清單，再把各場域 build 的 `school` 換成正式字串

給單人版負責人：

- [ ] 確認現有 payload 仍照送（不用改）
- [ ] 確認 `data.school` 會換成正式場域字串
