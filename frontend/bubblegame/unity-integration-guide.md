...
## 0.1 依遊戲生命週期要呼叫哪些 API

### 快速結論

| 時機 | API | Unity 是否必須呼叫 | 用途 |
|---|---|---:|---|
| 遊戲前 | `GET /health` | 建議 | 確認後端可連線；失敗時可先提示網路問題 |
| 遊戲前 | `GET /api/games` | 建議 | 取得目前可用遊戲，以及 DAT／DCCS／EFT 是否支援雙人版 |
| 遊戲中 | 無 | 否 | 後端目前沒有逐題、逐回合或心跳 API；資料先留在 Unity 記憶體或本機暫存 |
| 遊戲後 | `POST /api/sessions` | **是** | 每位學生送出一筆完整場次與 5 個核心統計 |
| 遊戲後（查詢／驗收） | `GET /api/students/{studentKey}/report?school=...` | 否 | 老師或學生端登入後查看已儲存的場次、彙總與趨勢 |

### 建議呼叫順序

1. **進入遊戲大廳或選擇遊戲前**：呼叫 `/health`。連線正常後呼叫 `/api/games`，依 `gameType` 和 `doubleCapable` 建立可選清單。這兩支都是公開 API，不需要 token。
2. **開始一局時**：在 Unity 記錄 `startTime`（Unix 毫秒）。雙人版另外產生一次 `pairId`，同一局的兩位學生共用它。
3. **遊戲進行中**：本地累積答對、答錯、正確率、遊玩時間與關卡數；不需要也不能把每題資料直接送到目前的後端 API。遊戲專屬統計可以保留在 payload，但後端目前只寫入 5 個核心統計。
4. **一位學生完成時**：記錄 `endTime`，呼叫一次 `POST /api/sessions`。成功收到 `201` 和 `sessionId` 後，才將該筆標記為已送出。
5. **雙人版完成時**：兩位學生各自呼叫一次 `POST /api/sessions`，每筆使用自己的 `grade`／`caseId`／統計，但帶相同的 `mode: "double"` 和 `pairId`。不要把兩位學生合併成一個 request。
6. **查詢結果時**：報表頁先以老師或學生帳密登入，再將登入回傳的 token 放進 `Authorization: Bearer <token>`，呼叫 `/report`。這是查詢端使用，不是 Unity 寫入時使用。

### 失敗與重送

- `POST /api/sessions` 回 `201`：視為成功，保存回傳的 `sessionId`。
- 網路錯誤或回 `500`：保留完整 payload，稍後重送；不要在尚未收到成功回應前丟棄資料。
- 回 `400`：通常是 `school`、時間戳記、遊戲名稱或必要 stat 錯誤，先修正資料再送，不要原樣無限重試。
- 回 `409`：同一場次已存在，視為後端已收過；可將本地資料標記為已完成。
- 後端沒有提供「開始場次」或「更新未完成場次」API，因此不要在開始時先送一筆空成績。

以下各節的 payload、雙人版規則、錯誤碼與場域代碼，都是上述流程的詳細規格。
